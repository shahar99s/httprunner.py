...
HttpRunner 以 `Workflow` 为核心，将任意测试场景抽象为有序步骤的集合。
...
- [Workflow](step_Workflow.py)：引用执行其它测试用例文件
...
- [request_with_Workflow_reference_test.py](../examples/postman_echo/request_methods/request_with_Workflow_reference_test.py)：用例中包含了引用其它测试用例的情况

### 用例执行器 HttpRunner

测试用例的具体执行都由 `HttpRunner` 完成，每个 Workflow 对应一个实例，在该实例中除了包含测试用例自身内容外，还会包含测试过程的 session 数据和最终测试结果 summary。
...
- run：该方法将被 pytest 发现，作为启动执行入口，依次执行所有测试步骤
...
在主流程中，HttpRunner 并不需要关注 step 的具体类型，统一都是调用 `step.run(self)`，具体实现逻辑都在对应 step 的 `run` 方法中。
